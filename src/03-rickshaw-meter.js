...
export function parseFare(fareString) {
  ...
  if(typeof fareString !== "string") return -1;
    ...
    let fare = Number.parseFloat(fareString);
...
export function roundFare(amount, decimalPlaces) {
  ...
  if(typeof amount !== "number" || decimalPlaces < 0 || !Number.isInteger(decimalPlaces)) return "";
    ...
    const nFixed = amount.toFixed(decimalPlaces);
    return nFixed;
  }
}
...
export function calculateSurge(baseFare, surgeMultiplier) {
  ...
  if(typeof baseFare === "number" && typeof surgeMultiplier === "number" && baseFare > 0 && surgeMultiplier > 0) {
    let surge = baseFare * surgeMultiplier;
...
export function findCheapestAndCostliest(...fares) {
  // Your code here
  const filtered = fares.filter((num) => typeof num === "number");
  if(filtered.length === 0) return null;
  else {
    let cheapest = Math.min(...filtered);
    let costliest = Math.max(...filtered);
    return {cheapest, costliest};
  }
}
...
export function getDistanceDifference(from, to) {
  ...
  if(Number.isNaN(intFrom) || Number.isNaN(intTo)) return -1;
  else {
    return Math.abs(intFrom - intTo);
  }
}
